# /srv/mikrotik_manager/backup_scripts/huawei_ma5800_especifico.py
# Script específico para OLTs Huawei MA5800, usando a sequência manual validada.
...
def realizar_backup(ip: str, usuario: str, porta: int, nome_provedor: str, nome_tipo_equip: str,
    ...
    logger.emit(f"Iniciando backup específico para Huawei MA5800: {nome_dispositivo} ({ip})...")
    ...
    try:
        ...
        child.sendline('\n')
        child.expect(prompt_config, timeout=10)
        ...
        child.expect(r'return', timeout=300) # Espera até o final da configuração
        full_config = child.before
        child.expect(prompt_config, timeout=10) # Limpa o buffer até o prompt final
        ...
        error_msg = f"Falha crítica no script específico: {e}"
