...
from script_helpers import close_pexpect_session, open_pexpect_session, ssh_host_key_options
from olt_cli_backup import _login, _try_enable, _send_collect


def _jump_host_for_group(group, device):
    if not group or not uses_jump_host(group, device=device) or not group.jump_host:
        return None
...
def main():
    ...
    try:
        ...
        child = open_pexpect_session(
            command,
            jump_host=_jump_host_for_group(group, device),
            timeout=35,
            encoding="utf-8",
            codec_errors="ignore",
            logger=None,
        ...
        _try_enable(child, [extra.get("enable_password"), password, device.username])
